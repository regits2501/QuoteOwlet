...
var OAuth = class extends Options {
  ...
  // set X session identifier (nonce)
  setNonce() {
  ...
  this.RequestTokenLeg = function() {
    ...
    requestTokenLeg.phases.other = function setVerifyCredentials() {
      var credentialOptions = {
        // verify credential(access token) options
        options: {
          path: "/1.1/account/verify_credentials.json",
          method: "GET",
          params: {
            // avalable params for this api endpoint (server decides which are used)
...
function twizClient() {
  let r = new twiz();
  const head = {
    // none of the 'this' references in 'r' are exposed to outside code
